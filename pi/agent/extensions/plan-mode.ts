...
import { DynamicBorder, keyHint, getMarkdownTheme } from "@earendil-works/pi-coding-agent";
...
type PlanKeyAction = "selectUp" | "selectDown" | "selectConfirm" | "selectCancel" | "selectPageUp" | "selectPageDown";

const PLAN_KEY_BINDINGS: Record<PlanKeyAction, string[]> = {
	selectUp: ["up", "k"],
	selectDown: ["down", "j"],
	selectConfirm: ["return", "enter"],
	selectCancel: ["escape", "ctrl+c", "q"],
	selectPageUp: ["pageup", "page-up", "ctrl+b"],
	selectPageDown: ["pagedown", "page-down", "ctrl+f"],
};

function planKeyMatches(keyData: string, action: PlanKeyAction): boolean {
	return PLAN_KEY_BINDINGS[action].some((key) => matchesKey(keyData, key));
}
...
class PlanSelectorComponent extends Container implements Focusable {
	...
	handleInput(keyData: string): void {
		if (planKeyMatches(keyData, "selectUp")) {
		...
		if (planKeyMatches(keyData, "selectDown")) {
			if (this.filteredPlans.length === 0) return;
			this.selectedIndex = this.selectedIndex === this.filteredPlans.length - 1 ? 0 : this.selectedIndex + 1;
			this.updateList();
			return;
		}
		if (planKeyMatches(keyData, "selectConfirm")) {
			const selected = this.filteredPlans[this.selectedIndex];
			if (selected) this.onSelectCallback(selected);
			return;
		}
		if (planKeyMatches(keyData, "selectCancel")) {
...
class PlanDetailOverlayComponent {
	...
	handleInput(keyData: string): void {
		if (planKeyMatches(keyData, "selectCancel")) { this.onAction("back"); return; }
		if (planKeyMatches(keyData, "selectConfirm")) { this.onAction("execute"); return; }
		if (planKeyMatches(keyData, "selectUp")) { this.scrollBy(-1); return; }
		if (planKeyMatches(keyData, "selectDown")) { this.scrollBy(1); return; }
		if (planKeyMatches(keyData, "selectPageUp")) { this.scrollBy(-this.viewHeight || -1); return; }
		if (planKeyMatches(keyData, "selectPageDown")) { this.scrollBy(this.viewHeight || 1); return; }
	}
